...
// 但是某些对象还是和其他对象不太一样。为了区分对象的类型，
// 我们用typeof操作符获取对象类型，它总是返回一个字符串
...
// 如果我们用typeof将无法分出null，Array和通常意义上的object －－ {}。

// 包装对象
// 除了这些类型外，JavaScript还提供了包装对象，熟悉Java的小伙伴肯定很清楚int和Integer这种暧昧关系。
// number、boolean和string都有包装对象。
// 没错，在JavaScript中，字符串也区分string类型和它的包装类型。包装对象用new创建：
...
// 5. typeof 操作符可以判断出number,boolean,string,function和undefined
// 6. 判断Array要使用Array.isArray(arr);
// 7. 判断null 请使用myVar === null;
