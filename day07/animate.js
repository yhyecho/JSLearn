// 动画
// 用JavaScript实现动画，原理非常简单：我们只需要以固定的时间间隔（例如，0.1秒），
// 每次把DOM元素的CSS样式修改一点（例如，高宽各增加10%），看起来就像动画了。

// 但是要用JavaScript手动实现动画效果，需要编写非常复杂的代码。
// 如果想要把动画效果用函数封装起来便于复用，那考虑的事情就更多了。
...
// 你可能会遇到，有的动画如slideUp()根本没有效果。这是因为jQuery动画的原理是逐渐改变CSS的值，
// 如height从100px逐渐变为0。但是很多不是block性质的DOM元素，
// 对它们设置height根本就不起作用，所以动画也就没有效果。

// 此外，jQuery也没有实现对background-color的动画效果，
// 用animate()设置background-color也没有效果。这种情况下可以使用CSS3的transition实现动画效果。
